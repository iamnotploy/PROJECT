...
export type HotelOption = { id: string; name: string; address: string };
...
export function RoomManagement({ hotels, initialTypes, initialRooms }: { hotels: HotelOption[]; initialTypes: RoomTypeManagementRow[]; initialRooms: RoomManagementRow[] }) {
  const [types, setTypes] = useState(initialTypes);
  const [roomRows, setRoomRows] = useState(initialRooms);
  ...
  const visibleTypes = types.filter((type) => type.hotelId === selectedHotelId);
  const rooms = roomRows.filter((room) => visibleTypes.some((type) => type.id === room.roomTypeId));

  function selectHotel(id: string) {
    setSelectedHotelId(id);
    setRoomForm((current) => ({ ...current, roomTypeId: types.find((type) => type.hotelId === id)?.id ?? "" }));
  }
  ...
  async function addRoomType(event: React.FormEvent<HTMLFormElement>) {
    ...
    await runAction(async () => {
      ...
      if (!selectedHotelId) throw new Error("กรุณาเลือกโรงแรมก่อนเพิ่มประเภทห้อง");
      const { data, error } = await supabase.from("room_types").insert({ hotel_id: selectedHotelId, name: typeForm.name, description: typeForm.description, base_price: Number(typeForm.basePrice), max_guests: Number(typeForm.maxGuests), size_sqm: typeForm.sizeSqm ? Number(typeForm.sizeSqm) : null, bed_description: typeForm.bedDescription, bathrooms: Number(typeForm.bathrooms), amenities: typeForm.amenities.split(",").map((item) => item.trim()).filter(Boolean) }).select("id, hotel_id, name, description, base_price, max_guests, size_sqm, bed_description, bathrooms, amenities").single();
      if (error) throw error;
      setTypes((current) => [...current, { id: data.id, hotelId: data.hotel_id, name: data.name, description: data.description ?? "", basePrice: Number(data.base_price), maxGuests: data.max_guests, sizeSqm: data.size_sqm, bedDescription: data.bed_description, bathrooms: data.bathrooms, amenities: data.amenities ?? [] }]);
  ...
  async function addRoom(event: React.FormEvent<HTMLFormElement>) {
    ...
    await runAction(async () => {
      ...
      setRoomRows((current) => [...current, { id: data.id, roomTypeId: data.room_type_id, roomTypeName: type?.name ?? "", roomNumber: data.room_number, floor: data.floor, status: data.status, housekeepingStatus: data.housekeeping_status, lastCleanedAt: data.last_cleaned_at }]);
  ...
  async function updateRoom(id: string, patch: { status?: string; housekeeping_status?: string }) {
    await runAction(async () => {
      ...
      if (error) throw error;
      setRoomRows((current) => current.map((room) => room.id === id ? { ...room, status: patch.status ?? room.status, housekeepingStatus: patch.housekeeping_status ?? room.housekeepingStatus } : room));
  ...
  async function deleteRoom(id: string) {
    ...
    await runAction(async () => {
      ...
      if (error) throw error;
      setRoomRows((current) => current.filter((room) => room.id !== id));
