...
export function RoomCard({ room, compact = false, hrefQuery = "" }: { room: Room; compact?: boolean; hrefQuery?: string }) {
  const detailHref = `/rooms/${room.id}${hrefQuery ? `?${hrefQuery}` : ""}`;
      ...
      <div className={compact ? "flex min-w-0 flex-col p-4" : "p-4.5"}>
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <h3 className="truncate text-base font-bold text-ink">{room.name}{room.roomNumber ? ` · ห้อง ${room.roomNumber}` : ""}</h3>
          </div>
        ...
        <div className="mt-4 flex items-end justify-between gap-3 border-t pt-3">
          ...
          <Link href={detailHref}><Button size="sm" variant="secondary">ดูห้องพัก <ArrowUpRight className="size-3.5" /></Button></Link>
        </div>
      </div>
    </article>
  );
